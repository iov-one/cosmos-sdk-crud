...
func (s Store) Index(o crud.Object) error {
...
func (s Store) QueryAll(sk crud.SecondaryKey) (primaryKeys [][]byte, err error) {
...
// Does not return an error if no result is found
func (s Store) Query(sk crud.SecondaryKey, start, end uint64, do func(primaryKey []byte) (keepGoing bool)) error {
...
// Does not return an error if no result is found
func (s Store) getPrimaryKeysFromIndex(sk crud.SecondaryKey, start uint64, end uint64, do func(primaryKey []byte) (stop bool)) error {
	...
	if err != nil {
		return fmt.Errorf("%w: %s", crud.ErrBadArgument, err)
	}
...
func (s Store) mapKey(secondaryKey crud.SecondaryKey, primaryKey []byte) (computedKey []byte, err error) {
	store, computedKey, err := s.kvStore(secondaryKey)
	if err != nil {
		return nil, err
	}
	if store.Has(primaryKey) {
		return nil, fmt.Errorf("%w: primary key %x in index %s", crud.ErrAlreadyExists, primaryKey, secondaryKey)
	}
...
func (s Store) unmapRawKeys(primaryKey []byte, encodedKeys [][]byte) error {
	for _, encKey := range encodedKeys {
		...
		if !store.Has(primaryKey) {
			return fmt.Errorf("%w: key %x was not found in index key prefixed store %x", crud.ErrNotFound, primaryKey, encKey)
		}
...
func (s Store) kvStore(sk crud.SecondaryKey) (store sdk.KVStore, computedKey []byte, err error) {
	// compute key
...
func (s Store) saveIndexList(primaryKey []byte, encodedKeys [][]byte) error {
	...
	if s.primaryKeysIndexes.Has(primaryKey) {
		return fmt.Errorf("%w: key %x already exists in index list store", crud.ErrAlreadyExists, primaryKey)
	}
...
func (s Store) deleteIndexList(primaryKey []byte) error {
	if !s.primaryKeysIndexes.Has(primaryKey) {
		return fmt.Errorf("%w: key %x in index list store", crud.ErrNotFound, primaryKey)
	}
