...
func Test_encodeDecodeIndexKey(t *testing.T) {
	...
	t.Run("encode/error max length", func(t *testing.T) {
		...
		_, err := encodeIndexKey(types2.SecondaryKey{
			ID:    0x1,
			Value: c,
		})
	...
	t.Run("encode/prefix check", func(t *testing.T) {
		...
		k, err := encodeIndexKey(types2.SecondaryKey{
			ID:    0x1,
			Value: c,
		})
		if err != nil {
		...
		k2, err := encodeIndexKey(types2.SecondaryKey{
			ID:    0x1,
			Value: c2,
		})
		if err != nil {
	...
	t.Run("encode/empty key", func(t *testing.T) {
		var ek1, ek2 []byte
		var dk1, dk2 types2.SecondaryKey
		var err error

		// Encode k1, a sk with empty value, into ek1
		k1 := types2.SecondaryKey{
			ID:    0x1,
			Value: make([]byte, 0),
		}
	...
	t.Run("decode/error minimum length", func(t *testing.T) {
		...
		if !errors.Is(err, types2.ErrInternal) {
			t.Fatalf("unexpected error: %s", err)
		}
	})
